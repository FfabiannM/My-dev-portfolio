            ...
            <div className="flex gap-3">
              ...
              {experience.link && experience.link !== "" && (
                <a href={experience.link} target="_blank" rel="noreferrer" className="hover:scale-110 transition-transform">
                  <span className="text-sm border border-gray-300 dark:border-gray-700 px-3 py-1 rounded-full whitespace-nowrap">Ver Web</span>
                </a>
              )}
            </div>
          </div>
          <p className="text-gray-600 dark:text-gray-400 font-sans mb-4">{experience.description}</p>
            ...
            {experience.information.map((info, idx) => (
              <div key={idx} className="flex flex-col gap-2 border-l-2 border-gray-300 dark:border-gray-700 pl-4">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1">
                  <h4 className="text-lg font-semibold text-gray-800 dark:text-gray-200">{info.puesto}</h4>
                  <span className="text-xs text-gray-500 bg-gray-100 dark:bg-zinc-800 px-2 py-1 rounded-lg w-fit">{info.dates}</span>
                </div>
